...
import { IApiResponse } from "../_interface/product.interface";
...
export default function ProductView({
  ...
  return (
    ...
    <div className={`flex flex-col`}>
      {/* Top - div */}
      <div className={`fixed top-0 left-0 right-0 z-10 flex flex-col w-full h-[18vh] border-b bg-white gap-6 shadow-md`}>
        {/* Top 영역중 가게이름과 아이콘 부모 Div */}
        <div className={`flex w-full`}>
          {/* Top --> shop Title Div */}
          <div className={`h-[8vh] w-3/5 flex items-end px-4 text-gray-800 font-bold text-xl`}>가게이름</div>
          {/* 아이콘 배너 */}
          <div className={`h-[8vh] w-2/5 flex justify-center items-end text-gray-800 font-bold text-xl gap-3`}>
            <AiOutlineExport />
            <SlMagnifier />
            <FiShoppingCart />
          </div>
      ...
      {/* Product Items Div */}
      <div className={`flex flex-col w-full pt-[20vh] py-1.5`}>
        {/* 이미지와 내용들을 나누는 div */}
        {data.map((v, i) => (
          <div className={`flex border-b-2 w-full h-[9rem] pt-1.5`} key={i}>
            ...
            <div className={`w-2/3 flex flex-col items-start pt-2 p-1`}>
              <div className={`font-bold text-gray-700 text-xl`}>{v.name}</div>
              {/* 할인된 금액이 있는지에 대한 조건 */}
              {v.discountPrice !== "0" ? (
                <>
                  <div className={`flex gap-2`}>
                    <div className={`text-red-500 line-through text-sm`}>{v.mainPrice.replace(/\B(?=(\d{3})+(?!\d))/g, ",")} 원</div>
                ...
                <div className={`text-sm text-gray-700`}>{v.mainPrice.replace(/\B(?=(\d{3})+(?!\d))/g, ",")} 원</div>
              )}
              <div className={`text-xs overflow-hidden text-ellipsis whitespace-nowrap max-w-[200px] pt-1.5`}>{v.content}</div>
            </div>
          </div>
        ))}
